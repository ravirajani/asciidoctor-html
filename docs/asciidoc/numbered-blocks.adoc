...
{asciidoctor-html} automatically adds a number of the form `chapnum.blocknum` to the
...
|example |automatic |thm{nbhy}number |E.g., "`Example {chapnum}.1`" (default) +
                                      Depends on block style if set +
                                      (see <<sec-example-blocks>>)

|image |automatic |fig{nbhy}number |E.g., "`Figure {chapnum}.1`"

|table |automatic |tbl{nbhy}number |E.g., "`Table {chapnum}.1`"

|olist |when style is `figlist` |fig{nbhy}number |E.g., "`Figure {chapnum}.2`"

|stem |with `numbered` option |eqn{nbhy}number |E.g., "`({chapnum}.1)`"
|listing |with `numbered` option |ltg{nbhy}number |E.g., "`Listing {chapnum}.1`"
|===
...
[lemma,title-suffix="<<bib-shao,Lemma 6.3>>"]
====
