...
====
[#lst-first]
. Level 1 list item of list with ID `lst-first`.
. Another level 1 list item.
.. A level 2 list item.
... A level 3 list item.
... Another level 3 list item.
...
You can change the type of marker used at any list level with the `shift` attribute.
For example, setting `[shift=3]` on the first level and `[shift=-1]`
on the second level gives <<ex-shift>>.

[#ex-shift]
.List with shift attribute
====
[shift=3]
...
[#lst-exercise-parts%roomy,shift=1]
...
[%roomy,continue=lst-exercise-parts,shift=1]
