...
You can display the markers inline with the `inside` option.
...
.. [[relative-li2]]A normal level 2 list item.
[%relative]
...
The `relative` option is compatible with <<sec-markers,custom markers>> where reference text such as <<relative-custom>> also gets resolved properly.
