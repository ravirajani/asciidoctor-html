= Ordered Lists
:bubblesort: pass:q[[.smallcaps]#BubbleSort#]
...
<<alg-bubblesort>> is taken from <<bib-cormen>>.

[algorithm#alg-bubblesort]
.{bubblesort}stem:[(A,n)]
====
This procedure sorts array stem:[A[1:n\]].

[pseudocode#bubblesort,reftext={bubblesort}]
. *for* stem:[i=1] *to* stem:[n-1]
.. *for* stem:[j=n] *downto* stem:[i+1]
... *if* stem:[A[j\] < A[j-1\]]
.... swap stem:[A[j\]] with stem:[A[j-1\]]
====

Let us also typeset the corresponding exercise from <<bib-cormen,p.46>>.
[exercise]
.Correctness of <<alg-bubblesort>>
====
...
[shift=1]
. Let stem:[A'] denote the array stem:[A] after {bubblesort}stem:[(A,n)] is executed.
To prove that {bubblesort} is correct, you need to prove that it terminates and that
====
