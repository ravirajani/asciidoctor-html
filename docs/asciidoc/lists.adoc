...
{asciidoctor-html} has some powerful new features for https://docs.asciidoctor.org/asciidoc/latest/lists/ordered/[ordered lists]
including automatic generation of reference text.

NOTE: This part has two https://docs.asciidoctor.org/asciidoc/latest/attributes/document-attributes/[document attributes] set, viz. the
standard boolean attribute `:sectnums:` and the custom attribute `:pagestyle: multi` which splits the sections into separate pages
by default.
...
on the second level, and `[markers=a)]` on the third, gives <<ex-markers>>.
...
[markers=a)]
... A level 3 list item, marked with a lowercase letter followed by a bracket.
... Another level 3 list item.
...
====
[bibliography]
. [[bib-cormen]]Cormen, Thomas H., Charles Eric Leiserson, Ronald L. Rivest, and Clifford Stein. 2022. _Introduction to Algorithms._ Fourth edition. Cambridge, Massachusetts: The MIT Press.
. [[bib-roughgarden]]Roughgarden, Tim. 2022. _Algorithms Illuminated : Omnibus Edition._ New-York ( N.Y): Soundlikeyourself Publishing.
====
...
You can display the markers inline with the `inside` option. In <<ex-inside>>, this option is applied
to a <<sec-bibliography>> <<sec-bibliography,bibliography list>>.

[#ex-inside]
.Bibliography list with inline markers
====
[bibliography%inside%roomy]
. Cormen, Thomas H., Charles Eric Leiserson, Ronald L. Rivest, and Clifford Stein. 2022. _Introduction to Algorithms._ Fourth edition. Cambridge, Massachusetts: The MIT Press.
. Roughgarden, Tim. 2022. _Algorithms Illuminated : Omnibus Edition._ New-York ( N.Y): Soundlikeyourself Publishing.
====
...
A list with the `relative` option displays its markers relative to the parent list item or relative to the chapter number if it is a top-level list.

[#ex-relative]
.List with relative numbering
====
[%relative]
. A level 1 list item of a list with option `relative`. The numbering is relative to the
chapter number.
. [[relative-li]]Another level 1 item.
[markers=A.]
.. [[relative-li2]]A level 2 item of a sublist with `markers=A.`.
[%relative]
... A list item of a level 3 relative list.
... [[relative-li3]]Another level 3 list item.

Reference text such as <<relative-li>>, <<relative-li2>> and <<relative-li3>> gets resolved appropriately.

The `relative` option can be combined with the <<sec-markers,markers attribute>> with appropriate resolution of reference text (e.g., <<relative-custom>>).

[%relative,markers=(1)]
. A level 1 list item of a relative list with `markers=(1)`.
. [[relative-custom]]Another level 1 item.
====

[#sec-real-example]
== Real Example

In this section, we reproduce an exercise from <<bib-cormen,p.46>>. It demonstrates many of the list
